...
set -o pipefail
# Uncomment for verbose debug tracing (prints each command before executing):
# set -x
...
# Reference Genome
REFERENCE_BASENAME="Zymo_D6332.custom.reference.fasta" # Just the filename
...
# Keyed by Genus_species since multiple Streptococcus species are present.
# Multi-contig genomes (Porphyromonas, Prevotella) group all contigs together.
Fusobacterium_nucleatum='Fusobacterium_nucleatum_genome'
Haemophilus_parainfluenzae='Haemophilus_parainfluenzae_genome'
Neisseria_subflava='Neisseria_subflava_genome'
Porphyromonas_gingivalis='Porphyromonas_gingivalis_contig_1 Porphyromonas_gingivalis_contig_2 Porphyromonas_gingivalis_contig_3 Porphyromonas_gingivalis_contig_4 Porphyromonas_gingivalis_contig_5'
Prevotella_nigrescens='Prevotella_nigrescens_contig_1 Prevotella_nigrescens_contig_2 Prevotella_nigrescens_contig_3 Prevotella_nigrescens_contig_4 Prevotella_nigrescens_contig_5 Prevotella_nigrescens_contig_6'
Rothia_dentocariosa='Rothia_dentocariosa_genome'
Schaalia_odontolytica='Schaalia_odontolytica_genome'
Streptococcus_mitis='Streptococcus_mitis_genome'
Streptococcus_mutans='Streptococcus_mutans_genome'
Streptococcus_parasanguinis='Streptococcus_parasanguinis_genome'
Streptococcus_salivarius='Streptococcus_salivarius_genome'
Veillonella_parvula='Veillonella_parvula_genome'
...
# --- Log all subsequent output to a file (in addition to stdout/stderr) ---
RUN_LOG="${OUTPUT_DIR}/run.log"
exec > >(tee -a "${RUN_LOG}") 2>&1
echo "Logging all output to: ${RUN_LOG}"
echo ""

# --- Write run_info.txt for reproducibility ---
RUN_INFO="${OUTPUT_DIR}/run_info.txt"
{
    ...
    echo "Script md5:        $(md5sum "$0" 2>/dev/null | awk '{print $1}')"
    ...
    echo "REFERENCE:         ${REFERENCE}"
    echo "REFERENCE md5:     $(md5sum "${REFERENCE}" 2>/dev/null | awk '{print $1}')"
    echo "Seqsum file:       ${BASECALLED_SEQ_SUMMARY}"
    echo ""
    echo "=== Parameters ==="
    ...
    echo "samtools:          $(samtools --version 2>/dev/null | head -n1 || echo 'NOT FOUND')"
    echo "seqtk:             $(seqtk 2>&1 | grep -i version | head -n1 || echo 'see seqtk binary')"
    echo "pigz:              $(pigz --version 2>&1 | head -n1 || echo 'NOT FOUND')"
    echo "awk:               $(awk --version 2>/dev/null | head -n1 || awk -W version 2>&1 | head -n1 || echo 'unknown awk')"
    echo "bash:              ${BASH_VERSION}"
} > "${RUN_INFO}"
echo "Wrote run info to: ${RUN_INFO}"
echo ""
...
# --- Pre-build minimap2 index if it doesn't already exist ---
# minimap2 re-indexes the reference on every run otherwise; building a .mmi
# once and reusing it saves time on repeat runs.
# NOTE: the bucket / reference directory is read-only, so the index is
# cached under MAIN_OUTPUT_ROOT (shared across all samples for this reference).
INDEX_CACHE_DIR="${MAIN_OUTPUT_ROOT}/reference_index_cache"
mkdir -p "${INDEX_CACHE_DIR}"
REFERENCE_MMI="${INDEX_CACHE_DIR}/${REFERENCE_BASENAME}.map-ont.mmi"
if [ ! -f "${REFERENCE_MMI}" ] || [ "${REFERENCE}" -nt "${REFERENCE_MMI}" ]; then
    echo "Building minimap2 index: ${REFERENCE_MMI}"
    minimap2 -x map-ont -d "${REFERENCE_MMI}" "${REFERENCE}"
    echo "Index built."
else
    echo "Using existing minimap2 index: ${REFERENCE_MMI}"
fi
echo ""
...
for TRMT in "${TRMT_LIST[@]}"; do
    ...
    minimap2 -ax map-ont -t "${THREADS}" "${REFERENCE_MMI}" "${TRMT_FASTQ_INPUT}" | \
    ...
    if [ -s "${ANALYSIS_ALL_LIST_OUT}" ]; then
        # Faster than grep -F -f for large seqsum files: hash the read ID list
        # and check exact equality against the read_id column.
        awk -F'\t' -v col="${COL_READ_ID_NUM}" \
            'NR==FNR{a[$1];next} ($col in a)' \
            "${ANALYSIS_ALL_LIST_OUT}" "${BASECALLED_SEQ_SUMMARY}" >> "${ANALYSIS_ALL_SEQSUM_OUT}"
    ...
    for ISO in "${ISO_LIST[@]}"; do
        ...
        if [ -s "${ANALYSIS_ISO_LIST_OUT}" ]; then
             # Faster than grep -F -f for large seqsum files (see comment above).
             awk -F'\t' -v col="${COL_READ_ID_NUM}" \
                 'NR==FNR{a[$1];next} ($col in a)' \
                 "${ANALYSIS_ISO_LIST_OUT}" "${BASECALLED_SEQ_SUMMARY}" >> "${ANALYSIS_ISO_SEQSUM_OUT}"
